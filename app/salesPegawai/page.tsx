"use client"
import SideNav from "../dashboardPegawai/SideNav"
import { BsFillPrinterFill, BsFillCaretDownFill, BsFillCaretUpFill } from "react-icons/bs"
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";
import Header from "../dashboardPegawai/Header";
import React, { useState, useCallback, useEffect } from "react"
import { auth } from '@/firebase'
import { onAuthStateChanged } from "firebase/auth";
import { useRouter } from 'next/navigation'
import Loading from "../dashboardPegawai/Loading"
                        ...
                        <div className="min-h-[200px] w-full bg-white py-4 flex flex-col space-y-2 px-6">
                            ...
                            <ul className="text-sm text-gray-500">
                                            {sale.products.map((item, index) => (
                                                <li key={index}>- {item.name}= {item.quantity} pieces == (Rp{item.amount})</li>
                                ))}
                                    </ul>
                                </div>
                                <div>
                                <h1 className="text-3xl font-bold text-blue-800">Rp. {sale.totalAmount.toLocaleString()}</h1></div>
                            </div>
